...
<!-- 				<div class="col-sm-2"> -->
<%-- 						<select id="dependCaseId" name="dependCaseId" class="selectpicker" value="${APIcaseNew.dependCaseId}" class="selectpicker"  style="width: 190px; height: 30px"> --%>
<!-- 							<option value="0">无</option> -->
<%-- 							<c:forEach items="${APIcaseList}" var="r"> --%>
<%-- 								<option value="${r.id }" <c:if test="${APIcaseNew.dependCaseId eq r.id}">selected="selected"</c:if>>${r.caseName }</option> --%>
<%-- 							</c:forEach> --%>
<!-- 						</select> -->
<!-- 				</div>	 -->
				<label class="col-sm-1 control-label">用例名称</label>
				<div class="col-sm-2">
						<input class="form-control" id="caseId" name="caseId" value="${APIcaseNew.id}" style="display:none"  type="text" >
						<input class="form-control" id="caseName" name="caseName" value="${APIcaseNew.caseName}" readonly="readonly"  type="text" placeholder="用例名称">
				</div>			
			</div>
			<div class="panel-body">
				<label class="col-sm-1 control-label">用例描述</label>
					<div class="panel-body" >
						<textarea class="form-control" rows="4" style="resize: none;" id="remark" name="remark" placeholder="用例描述">${APIcaseNew.remark}</textarea>
					</div>		
			</div>
			<div class="panel-body" align="center">
				...
				<div class="main" align="center">
						<table id="API_table" class="data-table" border="1" style="table-layout:fixed;">
							...
							<tbody>
								<c:forEach items="${APICaseRelations}" var="e2" varStatus="status">
									<tr id="row_api">	
										<td id="APIId" style="display:none">${e2.APIId}</td>										
										<td id="APIName" title="${e2.apiName}">${e2.apiName}</td>
										<td id="apiHost" style="display:none">${e2.apiHost}</td>
										<td id="apiUrl" title="${e2.apiUrl}">${e2.apiUrl}</td>
										<!-- 判断apiMethod -->
										<c:if test="${e2.apiMethod eq 'get'}"><td id="apiMethod" title="get">get</td></c:if>
										<c:if test="${e2.apiMethod eq 'post'}"><td id="apiMethod" title="post">post</td></c:if>
										<c:if test="${e2.apiMethod eq 'post1'}"><td id="apiMethod" title="post加密">post加密</td></c:if>										
										<td id="caseData" title="${e2.caseData}">${e2.caseData}</td>
										<td id="caseAssert" style="display:none">${e2.caseAssert}</td>
										<td id="caseParam" style="display:none">${e2.caseParam}</td>
										<td align="center">
											<button class="btn btn-info" data-toggle="modal" data-target="#editInterface" onclick="editAPI(this)">编辑</button>
											<button class="btn btn-primary up" onclick="moveUp(this)">上移</button>
											<button class="btn btn-danger" onclick="deletetbody(this)">删除</button>
											<button class="btn btn-primary down" onclick="moveDown(this)">下移</button>
										</td>
									</tr>
								</c:forEach>
							</tbody>				
				...
				<div align="center">	
							<button type="button" class="btn btn-success btn-lg" onclick="updateCase();">保存</button>
