...
<body>
	<div class="all">
		<form action="${ctx}/CaseNew/list" method="post">
			<div class="whole">
...
<!-- 					</select> 				 -->
				...
				<a class="btn btn-success" href="${ctx}/CaseNew/addCaseNew" >新增</a>			
			</div>
			<div class="main">
			...
			<div class="col-md-6">
				...
				<ul class="pagination">
					<!-- 
                      ...
                      -->
					<li><a
						href="${pageContext.request.contextPath}/CaseNew/list?caseName=${caseName}&pagenum=1">首页</a>
					</li>
					<!-- 如果还有前页就访问当前页码-1的页面， -->
					<c:if test="${CaseList.hasPreviousPage}">
						<li><a
							href="${pageContext.request.contextPath}/CaseNew/list?caseName=${caseName}&pagenum=${CaseList.pageNum-1}">
								<span>上一页</span>
						</a></li>
					</c:if>
					<li>
						<!--遍历所有导航页码，如果遍历的页码页当前页码相等就高亮显示，如果相等就普通显示  --> <c:forEach
							...
							<c:if test="${page_Nums!=CaseList.pageNum }">
								<li><a
									href="${pageContext.request.contextPath}/CaseNew/list?caseName=${caseName}&pagenum=${page_Nums}">${page_Nums}</a></li>
							</c:if>
						</c:forEach>
					</li>
					<!-- 如果还有后页就访问当前页码+1的页面， -->
					...
					<li><a
						href="${pageContext.request.contextPath}/CaseNew/list?caseName=${caseName}&pagenum=${CaseList.pages}">末页</a></li>
				</ul>
			</div>
		</div>
	</div>
